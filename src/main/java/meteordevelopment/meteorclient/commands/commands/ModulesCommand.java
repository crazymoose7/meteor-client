...
public class ModulesCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.executes(context -> {
            ...
            Modules.loopCategories().forEach(category -> {
                MutableText categoryMessage = new LiteralText("");
    ...
    private MutableText getModuleText(Module module) {
        ...
        MutableText finalModule = new LiteralText(module.title);
        if (!module.isActive()) finalModule.formatted(Formatting.GRAY);
        if (!module.equals(Modules.get().getGroup(module.category).getLast())) finalModule.append(new LiteralText(", ").formatted(Formatting.GRAY));
