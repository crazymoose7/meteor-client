...
public class HudEditorScreen extends WidgetScreen implements Snapper.Container {
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (moved) {
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                if (hovered != null) mc.openScreen(new HudElementScreen(theme, hovered));
                else mc.openScreen(new AddHudElementScreen(theme, lastMouseX, lastMouseY));
            }
        }
    ...
    public static void renderElements(MatrixStack matrices) {
        Hud hud = Hud.get();
        boolean inactiveOnly = Utils.canUpdate() && hud.active;

        HudRenderer.INSTANCE.begin(matrices);
