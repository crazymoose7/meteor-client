...
public enum SortPriority implements Comparator<Entity> {
    ...
    private static int sortAngle(Entity e1, Entity e2) {
        ...
        double e1yaw = Math.abs(Rotations.getYaw(e1) - mc.player.getYaw(mc.getTickDelta()));
        double e2yaw = Math.abs(Rotations.getYaw(e2) - mc.player.getYaw(mc.getTickDelta()));

        double e1pitch = Math.abs(Rotations.getPitch(e1) - mc.player.getPitch(mc.getTickDelta()));
        double e2pitch = Math.abs(Rotations.getPitch(e2) - mc.player.getPitch(mc.getTickDelta()));
